...
exports.registerVoter = async function (firstName, lastName, studentID, password) {
    ...
    try {
        ...
        if (userExists) {
            ...
            response.error = `Error! An identity for the user ${studentID} already exists in the wallet. Please enter
        ...
        const secret = await ca.register({ affiliation: 'org1', enrollmentID: studentID, role: 'client' }, adminIdentity);
        ...
        console.log(`Successfully registered voter ${firstName} ${lastName}. Use studentID ${studentID} to login above.`);
        let response = `Successfully registered voter ${firstName} ${lastName}. Use studentID ${studentID} to login above.`;
        return response;
    } catch (error) {
        console.error(`Failed to register user + ${studentID} + : ${error}`);
        let response = {};
        response.error = error;
        return response;
    }
