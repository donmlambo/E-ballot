...
// connect to the vice pres election file
const ballotDataPath2 = path.join(process.cwd(), './lib/data/vicePresidents.json');
const ballotDataJson2 = fs.readFileSync(ballotDataPath2, 'utf8');
const ballotData2 = JSON.parse(ballotDataJson2);

// connect to the sec general election file
const ballotDataPath3 = path.join(process.cwd(), './lib/data/secretaryGenerals.json');
const ballotDataJson3 = fs.readFileSync(ballotDataPath3, 'utf8');
const ballotData3 = JSON.parse(ballotDataJson3);

// connect to the sec general election file
const ballotDataPath4 = path.join(process.cwd(), './lib/data/presElection.json');
const ballotDataJson4 = fs.readFileSync(ballotDataPath4, 'utf8');
const ballotData4 = JSON.parse(ballotDataJson4);
...
class MyBallotContract extends Contract {
   ...
   */
    async init(ctx) {
        ...
        let voter2 = await new Voter('Chris', 'Brown','no162105y', '210462463');
        ...
        await ctx.stub.putState(voter2.studentID, Buffer.from(JSON.stringify(voter2)));
        ...
        if (currElections.length === 0) {
            ...
            let electionStartDate = await new Date(2020, 5, 16);
            let electionEndDate = await new Date(2020, 5, 16);
        ...
        let firstPresParty = await new Candidate(ctx, 'PartyA', ballotData.firstPartyBrief);
        let secondPresParty = await new Candidate(ctx, 'PartyB', ballotData.secondPartyBrief);
        let thirdPresParty = await new Candidate(ctx, 'PartyC', ballotData.thirdPartyBrief);
        let fourthPresParty = await new Candidate(ctx, 'PartyD', ballotData.FourthPartyBrief);
        let fifthPresParty = await new Candidate(ctx, 'PartyE', ballotData.fifthPartyBrief);

        let firstvicePresParty = await new Candidate(ctx, 'PartyA', ballotData2.firstPartyBrief);
        let secondvicePresParty = await new Candidate(ctx, 'PartyB', ballotData2.secondPartyBrief);
        let thirdvicePresParty = await new Candidate(ctx, 'PartyC', ballotData2.thirdPartyBrief);
        let fourthvicePresParty = await new Candidate(ctx, 'PartyD', ballotData2.FourthPartyBrief);
        let fifthvicePresParty = await new Candidate(ctx, 'PartyE', ballotData2.fifthPartyBrief);

        let firsttresGenParty = await new Candidate(ctx, 'PartyA', ballotData4.firstPartyBrief);
        let secondtresGenParty = await new Candidate(ctx, 'PartyB', ballotData4.secondPartyBrief);
        let thirdtresGenParty = await new Candidate(ctx, 'PartyC', ballotData4.thirdPartyBrief);
        let fourthtresGenParty = await new Candidate(ctx, 'PartyD', ballotData4.FourthPartyBrief);
        let fifthtresGenParty = await new Candidate(ctx, 'PartyE', ballotData4.fifthPartyBrief);

        let firstsecGenParty = await new Candidate(ctx, 'PartyA', ballotData3.firstPartyBrief);
        let secondsecGenParty = await new Candidate(ctx, 'PartyB', ballotData3.secondPartyBrief);
        let thirdsecGenParty = await new Candidate(ctx, 'PartyC', ballotData3.thirdPartyBrief);
        let fourthsecGenParty = await new Candidate(ctx, 'PartyD', ballotData3.FourthPartyBrief);
        let fifthsecGenParty = await new Candidate(ctx, 'PartyE', ballotData3.fifthPartyBrief);
        ...
        for (let i = 0; i < Candidates.length; i++) {
            ...
            await ctx.stub.putState(Candidates[i].Party, Buffer.from(JSON.stringify(Candidates[i])));
   ...
   *
   * @param ctx - the context of the transaction
   * @param Candidates - The different political parties and candidates you can vote for, which are on the ballot.
   ...
   */
    async createVoter(ctx, args) {
        ...
        let Candidates = JSON.parse(await this.queryByObjectType(ctx, 'contestant'));
   ...
   */
    async deleteMyBallot(ctx, myBallotId) {

        const exists = await this.myBallotExists(ctx, myBallotId);
        if (!exists) {
   ...
   */
    async readMyBallot(ctx, myBallotId) {
   ...
   */
    async myBallotExists(ctx, myBallotId) {
   ...
   * @param Party - the Id of the candidate the voter has selected.
   ...
   */
    async castVote(ctx, args) {
        ...
        let choices = args.picked;

        //check to make sure the election exists
        let electionExists = await this.myBallotExists(ctx, args.electionId);
        ...
        if (electionExists) {
            ...
            if (parsedCurrentTime >= electionStart && parsedCurrentTime < electionEnd) {
                ...
                let candidateExists = await this.myBallotExists(ctx, choices);
                if (!candidateExists) {
                    let response = {};
